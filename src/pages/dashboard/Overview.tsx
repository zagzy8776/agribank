...
export default function Overview() {
  ...
  const balanceQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return accounts.reduce((s, a) => s + (a.balance_cents / 100), 0);
    },
    enabled: !!user,
  });
  ...
  const txnsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return txs.slice(0, 5).map(tx => ({
        ...
        name: tx.description, amount: tx.amount_cents / 100,
        currency: tx.currency, date: new Date(tx.created_at).toLocaleString(),
        status: tx.status,
      }));
    },
    enabled: !!user,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Total Deposits</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-green-600">{fmtMoney(Math.round((txnsQuery.data?.filter((t:any)=>t.type==='credit').reduce((s:number,t:any)=>s+t.amount,0)||0)*100),'EUR')}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Total Withdrawals</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{fmtMoney(Math.round((txnsQuery.data?.filter((t:any)=>t.type==='debit').reduce((s:number,t:any)=>s+t.amount,0)||0)*100),'EUR')}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Pending</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-amber-600">{fmtMoney(Math.round((txnsQuery.data?.filter((t:any)=>t.status==='pending').reduce((s:number,t:any)=>s+t.amount,0)||0)*100),'EUR')}</div></CardContent></Card>
      </div>
    </div>
  );
}